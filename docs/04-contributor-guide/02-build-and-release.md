...
`src/sources/BUILD.bazel` 有 `archive`、`git`、`clawhub`、`sources` 四个 `rust_library`；根 `BUILD.bazel` 的 `foundation` 与 `skill_bom` 组成其余模块。每个库只列自己编译的源码和依赖；不要把 `src/sources/*.rs` 重新放入根库的 `srcs`，否则适配器变动会扩大失效范围。
